#!/usr/bin/env python3
"""
Real-time training progress monitor with progress bars.
Usage: python scripts/training_progress.py
"""
import os
import sys
import time
...
def monitor():
    ...
    print("=" * 80)
    print("TRAINING PROGRESS - Real-time Monitor")
    print("=" * 80)
    ...
    if gpu_used is not None:
        ...
        # GPU utilization bar
        gpu_bar = progress_bar(gpu_util, 100, width=30, filled_char='█', empty_char='░')
        print(f"  [{gpu_bar}] {gpu_util}%")
    ...
    if latest_epoch and config:
        ...
        # Epoch progress bar
        epoch_bar = progress_bar(latest_epoch, total_epochs, width=50, filled_char='█', empty_char='░')
        print(f"  [{epoch_bar}] {progress:.1f}%")
        ...
        if latest_epoch > 0 and elapsed_seconds > 0:
            ...
            if remaining_time > 0:
                remaining_bar = progress_bar(elapsed_seconds, total_time, width=50, filled_char='█', empty_char='░')
                print(f"  [{remaining_bar}] {elapsed_seconds/total_time*100:.1f}%")
    ...
    print("\n" + "=" * 80)
    print("Press Ctrl+C to exit. Auto-refresh every 3 seconds...")
