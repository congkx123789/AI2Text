"""
Training script with timestamp support.

Extends the base trainer to support training with word-level timestamps.
"""
...
class ASRTrainerWithTimestamps:
    """Trainer class for ASR model with timestamp prediction."""
    
    def __init__(self, config: dict, db: ASRDatabase):
        """Initialize trainer with timestamp support."""
        self.config = config
        self.db = db
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.logger = setup_logger('ASRTrainerWithTimestamps', config.get('log_file', 'training_timestamps.log'))
        
        # Timestamp training config
        self.use_timestamps = config.get('use_timestamps', True)
        self.timestamp_loss_weight = config.get('timestamp_loss_weight', 0.1)
    ...
    def _setup_model(self):
        """Setup model with timestamp support."""
            ...
            if self.use_amp:
                with torch.cuda.amp.autocast():
                    ...
                    if self.use_timestamps and timestamps is not None:
                        ...
                        for i in range(len(audio_features)):
                            if word_timestamps_list[i] is not None:
                                ...
                                # Align with actual output length
                        ...
                        if len(timestamp_targets) > 0:
                            ...
                            targets_tensor = torch.stack(padded_targets)  # (batch, time, 2)
                    ...
                    if torch.isnan(loss) or torch.isinf(loss):
                        self.logger.error(f"❌ Batch {batch_idx}: Loss is NaN/Inf!")
                        continue
                ...
                # Similar logic for non-AMP training
                logits, output_lengths, timestamps = self.model(
                    audio_features, audio_lengths, return_timestamps=self.use_timestamps
                )
                ...
                timestamp_loss = torch.tensor(0.0, device=self.device)
                if self.use_timestamps and timestamps is not None:
                    # Similar timestamp loss calculation as above
    ...
    # Add validation and other methods similar to base trainer...
    # (Implementation continues...)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train ASR model with timestamps')
    parser.add_argument('--config', type=str, default='configs/default.yaml',
                       help='Path to config file')
    parser.add_argument('--use_timestamps', action='store_true',
                       help='Enable timestamp training')
    ...
    config['use_timestamps'] = args.use_timestamps or config.get('use_timestamps', True)
    
    # Initialize database and trainer
    db = ASRDatabase(config.get('database_path', 'database/asr_training.db'))
    trainer = ASRTrainerWithTimestamps(config, db)
    ...
    # Create data loaders (similar to main training script)
    # ... (implementation continues)
