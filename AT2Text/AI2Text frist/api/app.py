...
async def transcribe_audio(
    ...
    try:
        ...
        # Load audio - librosa supports many formats including m4a, mp3, wav, flac, etc.
        try:
            # First try soundfile for common formats (faster)
            audio_data, sample_rate = sf.read(str(tmp_path))
            if len(audio_data.shape) > 1:
                audio_data = audio_data[:, 0]  # Mono
        except Exception as sf_error:
            # Fallback to librosa for formats like m4a, mp3, etc.
            try:
                logger.info(f"soundfile failed, trying librosa: {sf_error}")
                # 使用kaiser_best作为重采样类型，这是librosa内置的，不依赖resampy
                audio_data, sample_rate = librosa.load(
                    str(tmp_path), 
                    sr=16000,
                    mono=True,
                    res_type='kaiser_best'  # 使用内置重采样方法
                )
            except Exception as librosa_error:
                logger.error(f"Failed to load audio: {librosa_error}")
                raise HTTPException(
                    status_code=400, 
                    detail=f"无法读取音频文件。支持的格式: WAV, MP3, M4A, FLAC, OGG等。错误: {str(librosa_error)}"
                )
        
        # Preprocess audio
        if sample_rate != 16000:
            # 使用librosa的内置重采样，不依赖resampy
            audio_data = librosa.resample(
                audio_data, 
                orig_sr=sample_rate, 
                target_sr=16000,
                res_type='kaiser_best'  # 使用内置的kaiser_best方法，不依赖resampy
            )
        ...
        if use_lm and lm_path and Path(lm_path).exists():
            ...
            if results:
                ...
                if not text or text.strip() == '':
                    ...
                    text += f"前10个最常见的token: {', '.join([f'ID {tid}({tokenizer_cache.idx_to_char.get(tid, \"<unk>\")}) x {count}' for tid, count in top_tokens])}\n"
