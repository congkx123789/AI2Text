...
class ASRDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if self._cached_features is not None and idx in self._cached_features:
            ...
            # DEBUG (alignment + feature stats): print a few samples once
            # Helps catch misalignment between audio and transcript and bad scaling.
            if idx < 5:
                try:
                    print(f"[DEBUG] Sample {idx}")
                    print(f"        file_path: {row.get('file_path', 'N/A')}")
                    print(f"        raw_transcript: {row.get('transcript', 'N/A')}")
                    print(f"        language: {row.get('language', 'N/A')}")
                    print(f"        mel shape: {mel_spec.shape}, "
                          f"min={float(mel_spec.min()):.3f}, "
                          f"max={float(mel_spec.max()):.3f}, "
                          f"mean={float(mel_spec.mean()):.3f}")
                except Exception:
                    # Debug must never break training
                    pass
...
def collate_fn(batch: List[Dict]) -> Dict[str, torch.Tensor]:
    ...
    for i, item in enumerate(batch):
        ...
        # Get language ID (default to 0 if not present for backward compatibility)
