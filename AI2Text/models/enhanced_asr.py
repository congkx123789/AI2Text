...
class CrossModalAttention(nn.Module):
    ...
    def forward(self, audio_features: torch.Tensor, text_features: torch.Tensor,
        ...
        # Use Flash Attention (SDPA) for optimized computation
        # Prepare mask for SDPA with proper format handling
        attn_mask = None
        if text_mask is not None:
            # Convert mask to proper format for SDPA
            if text_mask.dtype != torch.bool and text_mask.dtype != torch.float16 and text_mask.dtype != torch.float32:
                # Convert integer mask to boolean
                attn_mask = (text_mask != 0).bool()
            else:
                if text_mask.dtype == torch.bool:
                    attn_mask = text_mask
                else:
                    # Float mask: convert 0/1 to bool for consistency
                    if text_mask.min() >= 0 and text_mask.max() <= 1:
                        attn_mask = (text_mask > 0.5).bool()
                    else:
                        attn_mask = text_mask
            
            # Ensure correct shape for broadcasting: (batch, 1, audio_len, text_len)
            if attn_mask.dim() == 2:
        ...
        # Flash Attention automatically handles scaling, softmax, and dropout
        # Numerical stability: SDPA handles softmax with high precision internally
        attended_audio = F.scaled_dot_product_attention(
            Q_audio, K_text, V_text,
            attn_mask=attn_mask,
        ...
        if audio_mask is not None:
            # Convert mask to proper format for SDPA
            if audio_mask.dtype != torch.bool and audio_mask.dtype != torch.float16 and audio_mask.dtype != torch.float32:
                # Convert integer mask to boolean
                attn_mask = (audio_mask != 0).bool()
            else:
                if audio_mask.dtype == torch.bool:
                    attn_mask = audio_mask
                else:
                    # Float mask: convert 0/1 to bool for consistency
                    if audio_mask.min() >= 0 and audio_mask.max() <= 1:
                        attn_mask = (audio_mask > 0.5).bool()
                    else:
                        attn_mask = audio_mask
            
            # Ensure correct shape for broadcasting: (batch, 1, text_len, audio_len)
            if attn_mask.dim() == 2:
                attn_mask = attn_mask.unsqueeze(0).unsqueeze(0)
            elif attn_mask.dim() == 3:
                attn_mask = attn_mask.unsqueeze(1)
        ...
        # Flash Attention automatically handles scaling, softmax, and dropout
        # Numerical stability: SDPA handles softmax with high precision internally
        attended_text = F.scaled_dot_product_attention(
            Q_text, K_audio, V_audio,
            attn_mask=attn_mask,
            dropout_p=self.dropout.p if self.training else 0.0,
            is_causal=False
        )
