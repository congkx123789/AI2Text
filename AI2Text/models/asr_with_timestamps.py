...
class TimestampHead(nn.Module):
    ...
    Modernized with SiLU activation for consistency with encoder.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        x = self.activation(self.linear1(x))  # SiLU instead of ReLU
        x = self.dropout(x)
        timestamps = self.linear2(x)
        # Ensure timestamps are non-negative (use clamp instead of ReLU for smoother gradients)
        timestamps = torch.clamp(timestamps, min=0.0)
        return timestamps
...
class ASRModelWithTimestamps(ASRModel):
    ...
    def forward(self, x: torch.Tensor, 
                ...
                language_ids: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor, Optional[torch.Tensor]]:
