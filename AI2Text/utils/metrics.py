...
def calculate_wer(references: List[str], hypotheses: List[str]) -> float:
    ...
    for ref, hyp in zip(references, hypotheses):
        # Normalize both: remove extra spaces and convert to lowercase for comparison
        # This handles BPE tokenizer which doesn't preserve spaces
        ref_normalized = ' '.join(ref.lower().split())
        hyp_normalized = ' '.join(hyp.lower().split())
        
        # If prediction is empty, count all reference words as errors
        if len(hyp_normalized.strip()) == 0:
            ref_words = ref_normalized.split()
            total_distance += len(ref_words)
